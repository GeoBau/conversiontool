...
# Load workbook with data_only=True to read calculated values
file_path = "Portfolio_Syskomp_pA_neu.xlsx"
print(f"Lade Excel-Datei (mit berechneten Werten): {file_path}")
wb_data = openpyxl.load_workbook(file_path, data_only=True)
ws_data = wb_data.active

# Load workbook normally for writing
...
# Pattern for x.x.x.x (e.g., 0.0.411.14 or 8.0.000.04)
...
for row_idx in range(1, ws.max_row + 1):
    ...
    if cell_i.value is None or str(cell_i.value).strip() == "":
        ...
        for col_idx in range(1, 27):
            # Read from data_only workbook to get calculated values
